...
def one_d_gauss_legendre_nodes_up_to_a_level(a_level: int) -> List[OneDNodesAndWeights]:
    """
    ...
    References
    ---------
    build based on https://en.wikipedia.org/wiki/Gauss%E2%80%93Legendre_quadrature
    and
    ...
    for i in range(a_level + 1):
        ...
        roots, weights = roots_legendre(polynomial_order)
        one_d_nodes_and_weights_list.append(OneDNodesAndWeights(i + 1, roots, weights))

    return one_d_nodes_and_weights_list


def sparse_gauss_legendre_quadrature(dim: int, level: int, weight_cut_off: float = None):
    """
    ...
    >>> dim = 1
    >>> level = 2
    >>> indices, grid_points, weights = sparse_gauss_legendre_quadrature(dim, level)
    ...
    """
    one_d_gh_nodes_and_weights_lists = one_d_gauss_legendre_nodes_up_to_a_level(level)
