...
class CorrelatedProjectionFilter(ExponentialFamilyProjectionFilter, ABC):
    def __init__(self,
                 dynamic_sde: SDE,
                 measurement_sde: SDE,
        ...
        Returns
        -------
        out : CorrelatedProjectionFilter
        ...
        self._symbolic_stats_for_heun_update = [self._required_statistics_symbolic[0],
                                                ...
                                                self._required_statistics_symbolic[9],
                                                ]  # h, hc, R, F3
        ...
        d = len(dynamic_sde.variables)

        signature = '(d)->(m)'
        if d == 1:
            signature = '()->(m)'

        self._all_stats = jit(jnp.vectorize(all_stat_funcs, signature=signature))
        ...
        self._stats_for_heun_update = jit(
            jnp.vectorize(heun_funcs, signature=signature))
    ...
    def parameters_sde(self, theta, eta_tilde, fisher, dy, bijection_params):
        ...
        for i, a_stat in enumerate(self._required_statistics_symbolic):
            ...
            start_index += num_ell
        eh, ehh, ehc, ehhc, eR, eAc, eF0, eF1, eF2, eF3, eF4 = tuple(e_matrices)
        ...
        eM_plus_h = (jnp.kron(self._I, evec_R_inv)) @ eF4
        eM = eM_plus_h - eh

        eZ = eF0 + jnp.kron(eF1, eM_plus_h.T) - jnp.kron(eh.T, eF1) - jnp.kron(ec, eF2.T) + jnp.kron(eN, eM.T) + eP

        # fisher_eigvalsh = jnp.linalg.eigvalsh(fisher)
        # eval_min = jnp.min(fisher_eigvalsh)
        # fisher = (eval_min < 0) * (-1.1 * eval_min * jnp.eye(fisher.shape[0])) + fisher

        drift = jnp.linalg.solve(fisher, eAc.squeeze() - 0.5 * eZ @ evec_R_inv) * self._dt
        diffusion = jnp.linalg.solve(fisher, (eN + eF3) @ eR_inv @ dy)
        ...
        for i, a_stat in enumerate(self._symbolic_stats_for_heun_update):
            ...
            e_matrices.append(e_all[start_index:start_index+num_ell].reshape(a_stat.shape))
            start_index += num_ell
        eh_aux, ehc_aux, eR_aux, eF3_aux = tuple(e_matrices)
        ...
        # fisher_aux_eigvalsh = jnp.linalg.eigvalsh(fisher_aux)
        # eval_min = jnp.min(fisher_aux_eigvalsh)
        # fisher_aux = (eval_min < 0) * (-1.1 * eval_min * jnp.eye(fisher_aux.shape[0])) + fisher_aux

        diffusion_aux = jnp.linalg.solve(fisher_aux, (eN_aux + eF3_aux) @ eR_inv_aux @ dy)
