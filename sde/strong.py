...
from utils.vectorized import mat_vec
...
def ito_euler_maruyama(f: Callable[[jnp.ndarray, float], jnp.ndarray],
                       g: Callable[[jnp.ndarray, float], jnp.ndarray],
                       x: jnp.ndarray,
                       t: float,
                       dt: float,
                       stochastic_increments: jnp.ndarray) -> jnp.ndarray:
    """
    ...
    return f(x, t) * dt + mat_vec(g(x, t), stochastic_increments)


def ito_euler_scalar_noise_increment(f: Callable[[jnp.ndarray, float], jnp.ndarray],
                                     g: Callable[[jnp.ndarray, float], jnp.ndarray],
                                     x: jnp.ndarray, t: float, dt: float,
                                     stochastic_increments: float) -> jnp.ndarray:
    """
    ...
    return f(x, t) * dt + g(x, t).squeeze() * stochastic_increments


def ito_milstein_scalar_noise_increment(f: Callable[[jnp.ndarray, float], jnp.ndarray],
                                        g: Callable[[jnp.ndarray, float], jnp.ndarray],
                                        x: jnp.ndarray, t: float, dt: float,
                                        stochastic_increments: jnp.ndarray) -> jnp.ndarray:
    """
...
def strongSRKW1(f: Callable[[jnp.ndarray, float], jnp.ndarray],
                g: Callable[[jnp.ndarray, float], jnp.ndarray],
                x: jnp.ndarray, t: float, dt: float,
                stochastic_increments: jnp.ndarray,
                scheme: ButcherTable = SRK2W1) -> jnp.ndarray:
    """
    ...
    fs = jnp.stack((f_0, f_1, f_2, f_3))
    gs = jnp.stack((g_0, g_1, g_2, g_3))

    itos = jnp.array([i1, i2[1, 1] / sdt, i2[1, 0] / dt, i3 / dt])

    dx = (scheme.a @ fs) * dt + ((scheme.b.T @ itos) @ gs)
    return dx


def strongSRKWm(f: Callable[[jnp.ndarray, float], jnp.ndarray],
                g: Callable[[jnp.ndarray, float], jnp.ndarray],
                x: jnp.ndarray, t: float, dt: float,
                stochastic_increments: jnp.ndarray,
                scheme: ButcherTable = SRK2Wm) -> jnp.ndarray:
    """
    ...
    dx = (scheme.a[0] * f_0 + scheme.a[1] * f_1) * dt \
         + g_0 @ (scheme.b[0, 0] * i1) \
         + jnp.trace(g_1 * scheme.b[1, 1] + g_2 * scheme.b[1, 2], axis1=0, axis2=2) * sdt

    return dx


def strongSRA_Wm(f: Callable[[jnp.ndarray, float], jnp.ndarray],
                 g: Callable[[jnp.ndarray, float], jnp.ndarray],
                 x: jnp.ndarray, t: float, dt: float,
                 stochastic_increments: jnp.ndarray,
                 scheme: ButcherTable = SRA3Wm) -> jnp.ndarray:
    """
...
def stratonovich_euler_heun(f: Callable[[jnp.ndarray, float], jnp.ndarray],
                            g: Callable[[jnp.ndarray, float], jnp.ndarray],
                            x: jnp.ndarray,
                            t: float,
                            dt: float,
                            stochastic_increments: jnp.ndarray) -> jnp.ndarray:
    """
    Compute Euler Heun scheme of Stratonovich SDE with possibly vector brownian increment.
    This corresponds to eq (12) of
    Gilsing and Shardlow, "SDELab: A package for solving stochastic differential equations in MATLAB"
    2007
    with nu = 0.
    ...
    g_x_t = g(x, t)
    x_aux = x + g_x_t @ stochastic_increments
    return f(x, t) * dt + 0.5 * (g_x_t + g(x_aux, t)) @ stochastic_increments

    # g_x_t = g(x, t)
    # f_x_t = f(x, t)
    # x_aux = x + f_x_t*dt + g_x_t @ stochastic_increments
    # return 0.5 * (f(x_aux, t) + f_x_t)* dt + 0.5 * (g_x_t + g(x_aux, t)) @ stochastic_increments
