...
from sde.strong import ito_euler_maruyama, \
    ito_milstein_scalar_noise_increment, \
    ito_euler_scalar_noise_increment, \
    strongSRKW1, strongSRKWm, strongSRA_Wm, \
    stratonovich_euler_heun


def _check_args(f: Callable[[jnp.ndarray, float], jnp.ndarray], g: Callable[[jnp.ndarray, float], jnp.ndarray],
                x0: jnp.ndarray, tspan: jnp.ndarray,
                dW: jnp.ndarray) -> Tuple[int, int, Callable[[jnp.ndarray, float], jnp.ndarray],
                                          Callable[[jnp.ndarray, float], jnp.ndarray], jnp.ndarray,
    ...
    if not jnp.isclose(min(jnp.diff(tspan)), max(jnp.diff(tspan)), rtol=1e-2):
    ...
    if gtest.ndim == 1:
        m = 1
    elif gtest.ndim in (2, 3):
        m = gtest.shape[-1]

    if gtest.shape[-2] != n:
        raise SDEValueError(message)
    elif gtest.ndim not in (1, 2, 3):
        raise SDEValueError(message)
    ...
    if m == 1 and dW.shape[0] != tspan.shape[0]:
        raise SDEValueError(message)
    elif m > 1 and (dW.shape[0], dW.shape[-1]) != (tspan.shape[0], m):
        raise SDEValueError(message)

    return n, m, f, g, x0, tspan, dW


def sde_solver(f: Callable[[jnp.ndarray, float], jnp.ndarray],
    ...
    At the moment, only Euler Maruyama Scheme that supports vectorization.
    ...
    if solver_type == SDESolverTypes.ItoEulerMaruyama:
        if m == 1:
            ...
            increment = jit(partial(ito_euler_maruyama, f=f, g=g, dt=dt))
        carry = x0
        inputs = [tspan, dw]
        ...
        gv = partial(jnp.vectorize, signature='(n)->(n,m)', excluded=(1,))(g)  # <- the g function need to be
        # parallelized
    ...
    elif solver_type == SDESolverTypes.StrEulerHeun:
        increment = jit(partial(stratonovich_euler_heun, f=f, g=g, dt=dt))
        carry = x0
        inputs = [tspan, dw]
